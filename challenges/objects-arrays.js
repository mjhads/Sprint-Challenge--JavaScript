...
const universities = [];
universities.push(`${this.university.sort(a, b) => {return a > b}`);
...
const animalNames = [];
zooAnimals.forEach(function(zooAnimal){animalNames.push(`${this.animal_name} ${this.scientific_name}`)});
...
const lowerCase = [];
zooAnimals.map(function (animals){lowerCase.push(`${this.animal_name.toLowerCase()}`)});
